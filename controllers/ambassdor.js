...
exports.register = async(req,res)=>{
    ...
    try {
        ...
        const verifyEmailToken = user.getVerifyEmail();

        await user.save();

        const verifyUrl = `${req.protocol}://${req.get("host")}/verify/email/${verifyEmailToken}`;

        const message = `You are receiving this email because you (or someone else) has requested the verify this email. Please click below link to verify \n\n ${verifyUrl}`;


        try {
            await sendEmail({
                email: user.email,
                subject: "Please Verify Email to countinue with Karo Startup Registration Process",
                message
            });
            ...
            user.verifyEmialExpire = undefined;
            await user.save();
